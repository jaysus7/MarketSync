...
| **Baseline on `staging`** | `6c44e64` — Phase 8 coherence, normalized My Day, Management composition, provider-first Identity, credit/affordability/vehicle fit, the dealer Social Scheduler, Studio editor, actionable identity review, restored Inventory Intelligence navigation, no-emoji cleanup, operational Social calendar, sold-vehicle protection, My Day runtime repair, per-network social captions, vehicle-fit next actions, and canonical Social provider outcomes are merged. CI is green. The current Social filter slice has focused coverage green. |
