...
| **Baseline on `staging`** | `0b088d5` — Phase 8.0 coherence, normalized My Day, canonical operational-attention producers and the Management shell are merged; CI is green. The suite is **1064/1064** with all six `check:*`; `staff_members.start_date` and the uninvited-email identity guard are applied to staging. |
