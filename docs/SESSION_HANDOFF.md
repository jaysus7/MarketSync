...
| **Next phase** | **Phase 7 — People + Academy + Dealer Launch.** Step 0 verified at `a3745e7`. Truth check complete; build slices 7.1–7.6 listed in `docs/PHASE7_PEOPLE_LAUNCH.md` §IMPLEMENTATION. |
| **Target branch** | `staging` (production deploys from `main` — see `render.yaml`) |
| **Baseline on `staging`** | `dfb0239` — **974/974 tests green, all six `check:*` green** (branch now at 1001/1001 with PR 7.6) |
