...
| **Baseline on `staging`** | `9328709` — Phase 8 coherence, normalized My Day, Management composition, provider-first Identity, credit/affordability/vehicle fit, the dealer Social Scheduler, Studio editor, actionable identity review, restored Inventory Intelligence navigation, no-emoji cleanup, operational Social calendar, sold-vehicle protection, My Day runtime repair, per-network social captions, and vehicle-fit next actions are merged. CI is green. The current Social provider-evidence slice has focused coverage green. |
