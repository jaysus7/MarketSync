...
| **Baseline on `staging`** | `78cca4c` — Phase 8 coherence, normalized My Day, Management composition, provider-first Identity, credit/affordability/vehicle fit, the dealer Social Scheduler, Studio editor, actionable identity review, restored Inventory Intelligence navigation, no-emoji cleanup, and the operational Social calendar are merged. CI is green. The current vehicle/campaign scheduler slice has focused behavioural coverage green. |
