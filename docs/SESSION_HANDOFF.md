...
| **Baseline on `staging`** | `addd44e` — Phase 8 coherence, normalized My Day, Management composition, provider-first Identity, credit/affordability/vehicle fit, the dealer Social Scheduler, Studio editor, actionable identity review, restored Inventory Intelligence navigation, no-emoji cleanup, operational Social calendar, sold-vehicle protection, My Day runtime repair, per-network social captions, vehicle-fit next actions, canonical Social provider outcomes, and scheduler filters are merged. CI is green. The current atomic Social editor slice has focused and staging runtime coverage green. |
