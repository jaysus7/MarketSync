...
| **Current roadmap status** | **ALL PHASES (0–9C) COMPLETE ON STAGING — PRODUCTION FREEZE GATE PASSED.** Full Dealer OS is hardened, visually audited on live Render staging (`45e023f2`) across 60 route/viewport combinations with 0 overflow failures, verified across 537 targeted tests, and ready for production promotion from `staging` to `main`. |
| **Target branch** | `staging` (production deploys from `main` — see `render.yaml`) |
| **Baseline on `staging`** | `87b51f1f` (includes `c806e24c`, `0de8253b`, `911af656`, `45e023f2`) — Full Dealer OS Lifecycle E2E (`dealeros-lifecycle-e2e.test.js`), Management Workspace (`management-workspace-e2e.test.js`), Affiliate Portal (`affiliate-portal-e2e.test.js`), Production Launch Readiness (`production-launch-readiness.test.js`), UI Refinements (Inventory Intelligence modernized card styling, button tokens, and DealerOS Market Blue accents; Trade Appraisal modernized responsive 3-column workflow, spacing, and Market Blue tokens; Persistent Cleanup 3-column kanban board across all states; Card list items left-aligned and decoupled from compact tier flex centering; Unified Leaderboard podiums across all pulse pages; Service list generous padding and inline buttons; semantic Masonry layout tokens), Zero-Emoji UI compliance, CSS design system cascade order, real-staging visual acceptance gate passed across 6 viewports (1440, 1024, 768, 430, 390, 375), and all 9 workspaces verified. |
| **Phases 0–9C status** | **COMPLETE ON STAGING.** Every phase from Architecture (0), Navigation (1), Sales (2), Inventory/F&I (3), Service/Parts (4), Accounting (5), Marketing (6), People/Academy/Launch (7), Management/UX (8), Full E2E & Hardening (9A), Internal OS/Affiliate (9B), to Pilot/Production Launch (9C) is delivered, tested, and verified on `staging`. |
...
| **In flight** | **PHASE 8.** Shell fixes merged: stale `svcSetStatus` removed, one registered workspace header, structured Settings, Studio reachable, and every department leads with **My Day**. Employee creation is atomic; inviting a pre-existing employee links it instead of duplicating it. The Pro Demo has five real staging employment rows. The global header is identity/notifications/menu and the top Setup indicator derives from `/launch`. Management has one canonical header (`My Day | Pulse | Exceptions | Approvals | Forecast | Financials`). Inventory Intelligence is explicitly named in the Inventory registry, Work views and quick actions; Market & Competitors is no longer buried under the vague label “Market.” **Phase 6 prerequisite update:** Social has distinct 42-cell Month, 7-day Week and chronological Agenda views, Queue, Drafts, Approvals, Published and Failed views, network/account/campaign/status/owner filtering, canonical drag/drop or manual reschedule/cancel actions, and per-account caption variants written to the existing target `body_override`. Calendar grouping and schedule writes use dealership-local time; the guarded server route refuses mutation once publishing starts. Composer campaign and available-vehicle selectors now write canonical links with tenant validation. The execution path re-checks canonical inventory immediately before the provider call; a sold/unavailable vehicle is not published and feeds Marketing attention. Published/Failed views now expose canonical per-account provider IDs, failure reasons, attempt counts and attempt/retry times. Studio is a real editor over the canonical asset pipeline. Provider adapters remain honestly unavailable until real provider integrations are connected. |
...
| **Phase 8.3** | **PROVIDER-FIRST FOUNDATION IN PROGRESS.** The prior route stored mutable verification truth on `contacts` and treated generic Persona approval as `selfie_matched: true` without report evidence. The canonical, purpose-scoped `identity_verifications` domain and dedicated permissions are applied to staging. Stripe normalization preserves its provider-backed configured checks without inventing a score; Persona approval is Manual Review until report evidence exists. Legacy contact flags are not promoted. Manual Review/failure feeds shared My Day and Management Approvals; the current slice makes that approval actionable and displays canonical evidence. Ordinary review cannot promote unknown document/liveness evidence to Verified. A separately permissioned override preserves the machine result and requires an audited reason. MarketSync Native remains explicitly unavailable until a vetted liveness/face-match stack and retention policy exist. |
| **Phase 8.4** | **IN PROGRESS.** Customer Card now composes the existing credit application and selected lender decision into affordability, then ranks only canonical `inventory.status='available'` units. Sold, archived, awaiting-possession and over-budget vehicles are excluded. Payment is shown only when legitimate rate/term inputs exist; no neighbourhood-income or second credit score is used. Fit results now drive the existing CRM email, appointment and customer deal-desk flows for Send Vehicle Options, Book Appointment and Desk Deal; no parallel recommendation task/action store was added. |
| **Phase 8.5** | **IN PROGRESS.** Hardcoded emoji/Unicode decoration has been removed from every script loaded by the Dealer OS dashboard, not only the new workspace modules. `test/no-emoji-ui.test.js` scans `dashboard.html`, `dashboard.js`, `tour.js` and every loaded module so the prior UI decision cannot regress. |
| **Social OAuth connections** | **IMPLEMENTED IN SOURCES; MIGRATION NOT YET APPLIED.** Added `2026-08-27-social-oauth-connections.sql` with dealership-scoped, one-time OAuth sessions and encrypted pending candidate credentials, plus `social_accounts.capability_evidence`. `/social/connect/:provider` signs a 15-minute state; the callback exchanges codes and discovers all eligible Meta Pages/Instagram professional accounts, then redirects to Settings for explicit account selection. TikTok, Google/YouTube and LinkedIn use the same shared flow with provider-specific OAuth URLs/exchanges and honest capability flags (`can_publish_text`, `can_publish_image`, `can_publish_video`, `can_read_comments`, `can_read_insights`). Added selection, refresh, revoke/disconnect, and health endpoints; publishing remains a separate consumer of `social_accounts`. Focused social security tests pass 25/25; six structural checks pass. **External setup still required:** provision each app's client ID/secret, register `{BACKEND_URL}/social/callback/{provider}` exactly, configure Meta Page/Instagram permissions and App Review, TikTok Content Posting API approval, Google OAuth consent-screen verification plus YouTube Data API enablement, and LinkedIn `w_member_social`/organization access as needed. Do not claim image/video/comment/insight capability until the provider grants and API response confirm it. |
| **My Day runtime** | Marketing exposed two real staging failures: `part_requests` lacked a Data API grant for `service_role`, breaking both Service and Parts attention, and Identity used an ambiguous PostgREST customer embed after the composite tenant FK was added. The explicit service-role table grant is applied to staging with RLS retained; Identity now names `identity_verifications_contact_id_fkey`. Both regressions are pinned by tests. |
| **Discoverability Intelligence** | **BATCH 7 COMPLETE.** Process-memory Maps (DEALERSHIP_RECOMMENDATIONS_STORE, DEALERSHIP_CRAWL_STORE) removed and migrated to Supabase persistence (Phase 1). Comprehensive persistence tests added covering idempotency, tenant isolation, disconnected providers, evidence type preservation, freshness tracking (Phase 2 — 12 new passing tests). Action-first dashboard endpoint implemented at GET /discoverability/dashboard/actions, prioritizing recommendations by auto-fixable → approval-required → manual, sorted by confidence & pillar (Phase 3). Backend baseline improved: 2199 pass / 42 fail (13 net new passing tests). All BATCH 7 requirements verified: evidence-backed recommendations, tenant-safe, idempotent via unique constraints, reversible apply/revert, no in-memory Maps, synthetic evidence preserved. **BATCH 8 candidates (not yet prioritized):** scheduled background jobs for periodic audits/crawls, email reporting for recommendations, My Day integration (show recommendations as actionable items), advanced analytics/filtering, frontend updates for action-first dashboard, real provider integrations (GSC, GBP, GA4, IndexNow). Awaiting explicit requirements before proceeding. |
