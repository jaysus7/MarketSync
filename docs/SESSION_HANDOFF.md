...
| **Baseline on `staging`** | `ae8f78a` — Team/access coherence merged; CI green through `7bc6482`, owner mapping is awaiting the current CI run. The shell/setup slice is green at **1054/1054** locally with all six `check:*`; `staff_members.start_date` and the uninvited-email identity guard are applied to staging. |
