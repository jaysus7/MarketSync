...
| **Baseline on `staging`** | `42ac214` — **1049/1049 tests green, all six `check:*` and CI green.** The current Team/access coherence slice is green at **1051/1051** locally; its additive `staff_members.start_date` migration is applied to staging. |
...
| **In flight** | **PHASE 8.0.** Shell fixes already merged: stale `svcSetStatus` runtime export removed, registered workspaces render one primary header, dead Master Configuration landing replaced by structured Settings, Marketing Studio is reachable, and all dealer departments lead with **My Day**. Employee creation is now atomic with dealership registration/invitation; the required legacy `staff_members.team` column is populated. Current slice removes the browser-local demo roster, makes People Team read canonical employment/account/training/manager/start-date state, makes Users & Access return its linked employee, and retains old People URLs as hidden redirects instead of a second header/engine. **Open prerequisite truth:** Phase 6's provider-evidenced publishing pipeline exists, but the approved Canva-style Studio and dealer-facing Social calendar/queue/composer are still gaps; do not call final Dealer OS complete until closed. |
