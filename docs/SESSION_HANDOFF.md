...
| **Baseline on `staging`** | `4936c14` — Team/access/header/setup coherence merged; CI green through `7bc6482` with newer runs queued. The Settings/actions/iconography slice is green at **1057/1057** locally with all six `check:*`; `staff_members.start_date` and the uninvited-email identity guard are applied to staging. |
