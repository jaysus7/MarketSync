...
| **Baseline on `staging`** | `b6619c6` — **1045/1045 tests green, all six `check:*` green** before the current Phase 8.0 identity slice. The slice is green at **1049/1049** locally. |
...
| **In flight** | **PHASE 8.0.** Shell fixes already merged: stale `svcSetStatus` runtime export removed, registered workspaces render one primary header, dead Master Configuration landing replaced by structured Settings, Marketing Studio is reachable, and all dealer departments lead with **My Day**. Current identity slice fixes the required `staff_members.team` omission that prevented canonical employment creation, makes registration/invitation atomic with employment, and seeds the demo team as real staff rows. **Open prerequisite truth:** Phase 6's provider-evidenced publishing pipeline exists, but the approved Canva-style Studio and dealer-facing Social calendar/queue/composer are still gaps; do not call final Dealer OS complete until closed. |
