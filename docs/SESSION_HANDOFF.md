...
| **In flight** | **PHASE 8.** Shell fixes merged: stale `svcSetStatus` removed, one registered workspace header, structured Settings, Studio reachable, and every department leads with **My Day**. Employee creation is atomic; inviting a pre-existing employee links it instead of duplicating it. The Pro Demo has five real staging employment rows. The global header is identity/notifications/menu and the top Setup indicator derives from `/launch`. Management has one canonical header (`My Day | Pulse | Exceptions | Approvals | Forecast | Financials`). **Phase 6 prerequisite update:** Social now has dealer-facing Calendar (month/week/agenda), Queue, Drafts, Approvals, Published and Failed views plus canonical reschedule/cancel actions and dealer-timezone display. The approved Canva-style Studio remains a gap: the current Studio is a discoverable media library/composer asset picker, not a design editor. |
