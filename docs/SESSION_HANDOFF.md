...
| **Baseline on `staging`** | `0470966` — Phase 8.0 coherence and the normalized My Day contract are merged; recent CI runs are green. The operational-attention slice is green at **1063/1063** locally with all six `check:*`; `staff_members.start_date` and the uninvited-email identity guard are applied to staging. |
...
| **In flight** | **PHASE 8.0.** Shell fixes merged: stale `svcSetStatus` removed, one registered workspace header, structured Settings, Studio reachable, and every department leads with **My Day**. Employee creation is atomic; inviting a pre-existing employee links it instead of duplicating it. The Pro Demo now has five real staging employment rows: its linked dealer admin plus Marcus, Sarah, David and Elena as truthful **Not invited** employees. People Team reads canonical employment/account/training/manager/start-date state; Users & Access returns its linked employee; old People URLs are hidden redirects. Current slice reduces the global header to identity/notifications/menu and adds a role-gated top Setup indicator derived from `/launch`, refreshed after real saves. **Open prerequisite truth:** Phase 6's provider-evidenced publishing pipeline exists, but the approved Canva-style Studio and dealer-facing Social calendar/queue/composer are still gaps; do not call final Dealer OS complete until closed. |
| **Phase 8.1** | **IN PROGRESS.** Login now lands on the correct department My Day for the six assignable dealer roles. The shared `/my-day` result carries the Phase 8 reference-only contract (`source_type`, `source_id`, department, title, priority, owner, due, next action, deep link, attention type). Service, Parts, Inventory and F&I now have server-side canonical attention producers; `MY_DAY_GAPS` is empty and source failure remains visible rather than reading as a quiet day. |
