...
| **Baseline on `staging`** | `524350f` — Phase 8 coherence, normalized My Day, Management composition, provider-first Identity, credit/affordability/vehicle fit, the dealer Social Scheduler, Studio editor, actionable identity review, and restored Inventory Intelligence navigation are merged. The preceding CI boundary is green. The current no-emoji cleanup is **1090/1090** locally with its dedicated loaded-module regression check. |
...
| **In flight** | **PHASE 8.** Shell fixes merged: stale `svcSetStatus` removed, one registered workspace header, structured Settings, Studio reachable, and every department leads with **My Day**. Employee creation is atomic; inviting a pre-existing employee links it instead of duplicating it. The Pro Demo has five real staging employment rows. The global header is identity/notifications/menu and the top Setup indicator derives from `/launch`. Management has one canonical header (`My Day | Pulse | Exceptions | Approvals | Forecast | Financials`). Inventory Intelligence is explicitly named in the Inventory registry, Work views and quick actions; Market & Competitors is no longer buried under the vague label “Market.” **Phase 6 prerequisite update:** Social has dealer-facing Calendar (month/week/agenda), Queue, Drafts, Approvals, Published and Failed views plus canonical reschedule/cancel actions. Studio is a real editor over the canonical asset pipeline (social formats, tenant-owned backgrounds, text/CTA/colors, server render, shared Social composer), and scheduling wall time resolves on the server against the dealership timezone with DST tests. Provider adapters remain honestly unavailable until real provider integrations are connected. |
