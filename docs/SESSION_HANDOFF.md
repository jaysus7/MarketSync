...
| **In flight** | **PHASE 8.** Shell fixes merged: stale `svcSetStatus` removed, one registered workspace header, structured Settings, Studio reachable, and every department leads with **My Day**. Employee creation is atomic; inviting a pre-existing employee links it instead of duplicating it. The Pro Demo has five real staging employment rows. The global header is identity/notifications/menu and the top Setup indicator derives from `/launch`. Management has one canonical header (`My Day | Pulse | Exceptions | Approvals | Forecast | Financials`). **Phase 6 prerequisite update:** Social has dealer-facing Calendar (month/week/agenda), Queue, Drafts, Approvals, Published and Failed views plus canonical reschedule/cancel actions. The current slice adds a real Studio creative editor over the canonical asset pipeline (social formats, tenant-owned backgrounds, text/CTA/colors, server render, shared Social composer) and resolves scheduling wall time on the server against the dealership timezone with DST tests. Provider adapters remain honestly unavailable until real provider integrations are connected. |
