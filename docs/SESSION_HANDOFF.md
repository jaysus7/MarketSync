...
| **Baseline on `staging`** | `aebe71c` — Phase 8.0 coherence, normalized My Day contract and canonical operational-attention producers are merged; CI is green. The Management shell slice is green at **1064/1064** locally with all six `check:*`; `staff_members.start_date` and the uninvited-email identity guard are applied to staging. |
