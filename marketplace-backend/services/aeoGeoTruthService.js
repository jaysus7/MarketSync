...
export function classifyCrawlerPolicy(robots, agents = ['googlebot', 'bingbot', 'oai-searchbot', 'gptbot', 'google-extended', 'applebot-extended']) { return Object.fromEntries(agents.map(agent => [agent, { agent, category: /gptbot|google-extended|applebot-extended/i.test(agent) ? 'training_or_extended_use' : 'search_or_retrieval', allowed: robots?.allowed ? robots.allowed(`https://example.com/`, agent) : null }])) }
