...
// discovery_summary / _terms / _intents / _enabled are BRANDING keys, not
// dealerships columns — the table has none of them. Selecting them made
// PostgREST reject the query, so every public site request failed. They are
// read from `branding` in siteContent() below, which is where they live.
...
async function buildSiteResponse(d) {
  const [inv, { data: team }, { data: interests }] = await Promise.all([
    // loadPublicInventory (restored from the staging rewrite) falls back to an
    // unfiltered read when the strict archived/possession filters return
    // nothing, which is what fixed dealers whose site showed no vehicles.
    loadPublicInventory(d.id),
  ...
  // `vehicles` and `count` are aliases the site frontend started reading
  // during the rewrite; keep them so nothing that consumes this breaks.
  return { site: siteContent(d), inventory, vehicles: inventory, count: inventory.length, team: publicTeam }
}
