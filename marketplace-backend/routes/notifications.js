...
export function classifyNotificationProduct(n) {
  ...
  if (type.startsWith('fb_') || type.includes('facebook') || type.includes('extension') || type === 'removal_required' || type === 'sold_vehicle' || type === 'pending_sale' || type.includes('inventory_sync') || title.includes('facebook') || title.includes('listing') || title.includes('extension') || page === 'inventory') {
    return 'facebook'
  }
...
const DEMO_PRODUCT_NOTIFICATIONS = {
  facebook: [
    { id: 'demo-fb-1', product: 'facebook', type: 'sold_vehicle', title: 'Sold Vehicle: 2022 Chevrolet Silverado 1500', body: 'Vehicle marked Sold in DMS. Facebook Marketplace listing removal required within 24 hours.', severity: 'action_required', status: 'open', needs_action: true, action_label: 'Remove Listing', action_page: 'inventory', action_filter: 'Silverado', read: false, created_at: new Date(Date.now() - 15 * 60000).toISOString() },
    { id: 'demo-fb-2', product: 'facebook', type: 'pending_sale', title: 'Pending Sale: 2023 Ford F-150 Lariat', body: 'Deposit placed on unit. Consider pausing active Facebook ads to prevent duplicate buyer inquiries.', severity: 'warning', status: 'open', needs_action: true, action_label: 'View Listing', action_page: 'inventory', action_filter: 'F-150', read: false, created_at: new Date(Date.now() - 90 * 60000).toISOString() },
    { id: 'demo-fb-3', product: 'facebook', type: 'extension_disconnected', title: 'Chrome Extension Session Expired', body: 'MarketSync AutoPoster Chrome extension requires session refresh before next scheduled auto-post batch.', severity: 'critical', status: 'open', needs_action: true, action_label: 'Connect Extension', action_url: 'https://chromewebstore.google.com/detail/marketsync/mfoaodaoipaalloccolophjhblgikada', read: false, created_at: new Date(Date.now() - 240 * 60000).toISOString() },
    { id: 'demo-fb-4', product: 'facebook', type: 'inventory_sync', title: 'Inventory Sync Complete', body: '18 vehicles updated from DMS feed. 4 new frontline arrivals ready to post.', severity: 'info', status: 'acknowledged', needs_action: false, action_label: 'Review Inventory', action_page: 'inventory', read: true, created_at: new Date(Date.now() - 480 * 60000).toISOString() }
  ],
  video: [
