...
test('Design Studio header is split into two stacked layers: identity on top, toolbar actions below', () => {
  const identityHeader = studio.match(/<header class="h-14 bg-white dark:bg-slate-900[\s\S]*?<\/header>/)?.[0] || ''
...
test('translucent indigo/blue accents in the toolbar get a solid, readable light-mode pairing', () => {
  ...
  assert.match(studio, /bg-indigo-600 text-white dark:bg-indigo-600\/20 dark:text-indigo-400/, 'the Design Studio badge must be readable in light mode')
