...
test('every frontend asset edited by the staging repair has the release cache version', () => {
  ...
  for (const asset of ['css/marketsync-theme.css', 'css/ms-liquid-glass.css', 'js/modules/dashboard-part17.js']) {
    assert.ok(html.includes(`${asset}?v=${builderVersion}`), `${asset} builder cache version is stale`)
  }

  assert.equal((loader.match(new RegExp(`js/modules/studio/studio-shell\\.js\\?v=${version}`, 'g')) || []).length, 2)
})
