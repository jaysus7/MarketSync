...
test('only posted journals count toward any balance', () => {
  ...
  // Nothing may read journal_lines without going through that helper.
  const directReads = [...arap.matchAll(/from\('journal_lines'\)/g)].length
  assert.equal(directReads, 1, `journal_lines must be read only inside postedLinesFor, saw ${directReads}`)
})
...
test('it composes accounting endpoints and introduces none', () => {
  ...
  const WRITES = ['/expenses/${id}/approve', '/expenses/${id}/pay']
