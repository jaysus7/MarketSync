...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('pr-pdf-btn')?.addEventListener('click', () => {
    const inner = document.getElementById('price-report-inner');
    if (!inner) return;
    const win = window.open('', '_blank');
    win.document.write(`<!DOCTYPE html><html><head><title>Price Report</title>
      <style>
        body{font-family:system-ui,sans-serif;padding:32px;color:#1e293b;max-width:700px;margin:0 auto}
        h3{font-size:1.2rem;font-weight:800;margin:0 0 4px}
        .strip{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin:16px 0}
        .tile{background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:12px;text-align:center}
        .tile .label{font-size:10px;text-transform:uppercase;font-weight:700;letter-spacing:.05em;color:#94a3b8;margin-bottom:4px}
        .tile .val{font-size:1.3rem;font-weight:900}
        table{width:100%;border-collapse:collapse;font-size:12px;margin-top:8px}
        th{text-align:left;padding:6px 8px;border-bottom:2px solid #e2e8f0;font-size:10px;text-transform:uppercase;letter-spacing:.05em;color:#64748b}
      ...
      ${inner.querySelector('#pr-title')?.outerHTML || ''}
      ${inner.querySelector('#pr-subtitle')?.outerHTML || ''}
      ${inner.querySelector('.grid.grid-cols-3')?.outerHTML || ''}
      <h4 style="font-size:11px;text-transform:uppercase;font-weight:700;letter-spacing:.05em;color:#94a3b8;margin:16px 0 8px">Comparable Vehicles on Lot</h4>
      ${inner.querySelector('table')?.outerHTML || ''}
      ${inner.querySelector('.note, p.text-xs')?.outerHTML || ''}
      <p class="note" style="margin-top:24px;border-top:1px solid #e2e8f0;padding-top:12px">Generated by MarketSync AI Boost · ${new Date().toLocaleDateString()}</p>
    </body></html>`);
    win.document.close();
    win.focus();
...
async function openPriceReport(inventoryId) {
  ...
  try {
    ...
    const { vehicle, comps, median: med, pct_diff } = await res.json();
    ...
    if (pct_diff != null) {
      ...
      diffEl.title = over ? 'Priced above lot median' : 'Priced below lot median';
    } else {
      diffEl.textContent = '—';
      diffEl.className = 'text-xl font-black text-slate-400';
    }

    document.getElementById('pr-comp-range').textContent =
      `(±2 years of ${vehicle.year} ${vehicle.make} ${vehicle.model})`;
    document.getElementById('pr-comp-count').textContent = comps.length;
    ...
    if (ctx && typeof Chart !== 'undefined') {
      ...
      const allPrices = [...compPrices, yourPrice].sort((a, b) => a - b);
      const labels = allPrices.map((p, i) => p === yourPrice ? 'This vehicle' : `Comp ${i + 1}`);
      const colors = allPrices.map(p => p === yourPrice ? '#6366f1' : '#cbd5e1');
      const borderColors = allPrices.map(p => p === yourPrice ? '#4f46e5' : '#94a3b8');
    ...
    // Comp table
    const tbody = document.getElementById('pr-comp-table');
    if (comps.length === 0) {
      tbody.innerHTML = '<tr><td colspan="4" class="px-3 py-4 text-slate-400 italic text-center">No comparable vehicles found on lot</td></tr>';
    } else {
      tbody.innerHTML = comps.map(c => {
        const cPrice = Number(c.price);
        const vs = med ? Math.round(((cPrice - med) / med) * 100) : null;
        const vsText = vs != null ? `<span class="${vs > 0 ? 'text-red-500' : vs < 0 ? 'text-amber-500' : 'text-slate-400'}">${vs > 0 ? '+' : ''}${vs}%</span>` : '—';
        const cLabel = [c.year, c.make, c.model, c.trim].filter(Boolean).join(' ');
        return `<tr>
          <td class="px-3 py-2 text-slate-800 dark:text-slate-200">${cLabel}</td>
          <td class="px-3 py-2 text-slate-500 capitalize">${c.condition || '—'}</td>
          <td class="px-3 py-2 text-right font-mono font-bold text-slate-900 dark:text-white">${fmt(c.price)}</td>
          <td class="px-3 py-2 text-right font-mono">${vsText}</td>
        </tr>`;
      }).join('');
    }
