...
function renderStudioIconLibrary(query = '') { return studioCatalogButtons(STUDIO_ICON_LIBRARY, query, item => `<button type="button" onclick="studioAddIcon('${item.id}')" title="${escS(item.label)}" class="aspect-square rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-indigo-500/20 border border-slate-300 dark:border-slate-700 flex flex-col items-center justify-center gap-1 text-slate-900 dark:text-white"><span class="text-2xl leading-none" aria-hidden="true">${STUDIO_ICON_SYMBOLS[item.name] || '✦'}</span><i class="fa-solid fa-${item.name} hidden" aria-hidden="true"></i><span class="text-[8px] truncate w-full px-1">${escS(item.label)}</span></button>`); }
...
function studioAddIcon(id) { const item = STUDIO_ICON_LIBRARY.find(icon => icon.id === id); if (!item || !window.__studioAdapter) return; loadStudioIconFont(); window.__studioAdapter.addText(STUDIO_ICON_SYMBOLS[item.name] || '✦', { fontSize: 110, fontWeight: '900', name: `Icon: ${item.name}`, iconName: item.name }); }
