...
async function searchStudioGifs() {
  ...
  results.innerHTML = '<div class="col-span-3 p-3 text-center text-xs text-slate-500 dark:text-slate-400">Searching animated assets…</div>';
  try {
...
function renderStudioWorkspaceHtml(designName, scene) {
  return `
    ...
    <footer data-studio-region="footer" class="h-16 flex-shrink-0 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 px-3 flex items-center gap-2 overflow-x-auto z-30">
      ...
      <button onclick="addStudioPage()" class="px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-black">+ Page</button>
...
function renderStudioToolPanelContent(tool) {
  ...
  if (tool === 'layers') {
    ...
    return `<div class="p-4 space-y-3"><div><h3 class="text-xs font-black uppercase tracking-wider text-slate-600 dark:text-slate-300">Layers</h3><p class="text-[10px] text-slate-500 dark:text-slate-400 mt-1">Select and reorder the current page. Groups and component children remain part of the document model.</p></div><div class="grid grid-cols-2 gap-2"><button type="button" onclick="addStudioStructure('component')" class="px-2 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-[10px] font-black text-white">+ Component</button><button type="button" onclick="addStudioStructure('repeater')" class="px-2 py-2 rounded-lg bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-[10px] font-black text-slate-900 dark:text-white">+ Repeater</button></div><div class="space-y-1.5 max-h-[55vh] overflow-y-auto">${rows || '<p class="text-xs text-slate-500">No layers yet.</p>'}</div>${structures.length ? `<div class="pt-2 border-t border-slate-200 dark:border-slate-800"><div class="text-[10px] font-black uppercase text-sky-400 mb-1">Structured elements</div>${structures.map((item, index) => `<button type="button" onclick="editStudioStructure(${index})" class="w-full flex items-center justify-between text-left text-xs text-slate-300 py-1 hover:text-white"><span>${item.type === 'repeater' ? '↻' : '◇'} ${escS(item.name)}</span><span class="text-[10px] text-sky-400">Edit</span></button>`).join('')}</div>` : ''}</div>`;
  }
  if (tool === 'templates') {
    return `
    ...
    return `<div class="p-4 space-y-3"><div><h3 class="text-xs font-black uppercase tracking-wider text-slate-900 dark:text-white">Icons</h3><p class="text-[10px] text-slate-500 dark:text-slate-400 mt-1">Free SVG icons from open icon libraries. Emojis remain available in Stickers.</p></div><select id="studio-icon-library-select" onchange="filterStudioIcons()" class="w-full px-3 py-2 rounded-xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white border border-white/10 text-xs">${Object.entries(STUDIO_ICON_LIBRARIES).map(([key,[label]]) => `<option value="${key}">${label}</option>`).join('')}</select><input id="studio-icon-query" oninput="filterStudioIcons()" placeholder="Search 120 icons..." class="w-full px-3 py-2 rounded-xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder:text-slate-500 border border-white/10 text-xs"><div id="studio-icon-library" class="grid grid-cols-4 gap-2 max-h-[65vh] overflow-y-auto">${renderStudioIconLibrary()}</div></div>`;
  } else if (tool === 'text') {
    return `
...
function editStudioStructure(index) {
  ...
  crmOverlay(`<form onsubmit="event.preventDefault(); saveStudioStructure(${index})" class="p-5 space-y-4 max-w-md"><div><h3 class="text-lg font-black text-slate-900 dark:text-white">${item.type === 'repeater' ? 'Repeater mapping' : 'Component settings'}</h3><p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Configure the reusable definition used by this design.</p></div><label class="block text-xs font-bold text-slate-300">Name<input id="studio-structure-name" value="${escS(item.name || '')}" class="mt-1 w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white"></label>${item.type === 'repeater' ? `<label class="block text-xs font-bold text-slate-300">Collection<select id="studio-structure-collection" class="mt-1 w-full px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white"><option value="inventory" ${item.collection === 'inventory' ? 'selected' : ''}>Inventory</option><option value="customers" ${item.collection === 'customers' ? 'selected' : ''}>Customers</option><option value="services" ${item.collection === 'services' ? 'selected' : ''}>Service offers</option></select></label><div><div class="text-xs font-bold text-slate-300 mb-2">Mapped fields</div><div class="grid grid-cols-2 gap-2">${fields.map(field => `<label class="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400"><input type="checkbox" value="${field}" ${item.template?.fields?.includes(field) ? 'checked' : ''}>${field}</label>`).join('')}</div></div>` : ''}<div class="flex justify-end gap-2"><button type="submit" class="px-4 py-2 rounded-xl bg-indigo-600 text-xs font-black text-white">Save mapping</button></div></form>`);
}
  ...
  return `
    ...
    <div class="space-y-3">
      <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-3">
        <label class="text-[10px] font-bold text-slate-500">Layer name<input value="${escS(object?.msData?.name || '')}" placeholder="Untitled layer" onchange="renameStudioLayer(this.value)" class="mt-1 w-full px-2 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white"></label>
        <div class="grid grid-cols-2 gap-2"><label class="text-[10px] font-bold text-slate-500">X<input type="number" value="${Math.round(object?.left || 0)}" onchange="studioSetObjectGeometry('left', this.value)" class="mt-1 w-full px-2 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white"></label><label class="text-[10px] font-bold text-slate-500">Y<input type="number" value="${Math.round(object?.top || 0)}" onchange="studioSetObjectGeometry('top', this.value)" class="mt-1 w-full px-2 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white"></label><label class="text-[10px] font-bold text-slate-500">Width<input type="number" value="${Math.round(object?.getScaledWidth?.() || object?.width || 0)}" onchange="studioSetObjectGeometry('width', this.value)" class="mt-1 w-full px-2 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white"></label><label class="text-[10px] font-bold text-slate-500">Height<input type="number" value="${Math.round(object?.getScaledHeight?.() || object?.height || 0)}" onchange="studioSetObjectGeometry('height', this.value)" class="mt-1 w-full px-2 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white"></label></div>
        ...
        <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400">Animation<select onchange="studioSetAnimation(this.value)" class="mt-1 w-full px-2 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white"><option value="none">None</option><option value="float">Float</option><option value="pulse">Pulse</option><option value="spin">Spin</option><option value="bounce">Bounce</option><option value="fade">Fade</option></select></label>
      </div>
      ${isText ? `<div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-2"><div class="text-[10px] font-black uppercase text-slate-500">Typography</div><label class="block text-[10px] text-slate-500">Font family<input value="${escS(object?.fontFamily || 'Manrope')}" onchange="studioSetTextStyle('fontFamily', this.value)" class="mt-1 w-full px-2 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white"></label><div class="grid grid-cols-3 gap-2"><label class="text-[10px] text-slate-500">Size<input type="number" min="6" value="${Math.round(object?.fontSize || 36)}" onchange="studioSetTextStyle('fontSize', Number(this.value))" class="mt-1 w-full px-2 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white"></label><label class="text-[10px] text-slate-500">Line height<input type="number" min="0.5" max="3" step="0.05" value="${Number(object?.lineHeight || 1.08).toFixed(2)}" onchange="studioSetTextStyle('lineHeight', Number(this.value))" class="mt-1 w-full px-2 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white"></label><label class="text-[10px] text-slate-500">Spacing<input type="number" min="-200" max="800" value="${Math.round(object?.charSpacing || 0)}" onchange="studioSetTextStyle('charSpacing', Number(this.value))" class="mt-1 w-full px-2 py-1.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-xs text-slate-900 dark:text-white"></label></div><div class="grid grid-cols-3 gap-1">${['left','center','right'].map(value => `<button type="button" onclick="studioSetTextStyle('textAlign','${value}')" class="py-1.5 rounded-lg ${object?.textAlign === value ? 'bg-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-800'} text-[10px] font-bold">${value[0].toUpperCase() + value.slice(1)}</button>`).join('')}</div><div class="grid grid-cols-2 gap-2"><button type="button" onclick="studioTransformText('uppercase')" class="py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-[10px] font-bold">UPPERCASE</button><button type="button" onclick="studioTransformText('lowercase')" class="py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-[10px] font-bold">lowercase</button></div></div>` : ''}
...
async function openStudioRevisionHistory() {
  ...
  try {
    ...
    const rows = revisions.length ? revisions.map(rev => `<div class="flex items-center justify-between gap-3 p-3 rounded-xl bg-white dark:bg-slate-900/60 border border-slate-300 dark:border-slate-700"><div class="min-w-0"><div class="text-sm font-bold text-slate-900 dark:text-white">Revision ${rev.revision_number}</div><div class="text-[11px] text-slate-500 dark:text-slate-400 truncate">${escS(rev.change_summary || 'Saved draft')} · ${new Date(rev.created_at).toLocaleString()}</div></div><button type="button" onclick="restoreStudioRevision('${rev.id}')" class="shrink-0 px-3 py-1.5 rounded-lg bg-slate-200 dark:bg-slate-700 hover:bg-indigo-600 text-xs font-bold text-slate-900 dark:text-white">Restore</button></div>`).join('') : '<p class="text-sm text-slate-500 dark:text-slate-400">No saved revisions yet.</p>';
    if (typeof crmOverlay === 'function') crmOverlay(`<div class="p-5 space-y-4 max-w-xl"><div class="flex items-center justify-between"><h3 class="text-lg font-black text-slate-900 dark:text-white">Design history</h3><span class="text-xs text-slate-500 dark:text-slate-400">${revisions.length} checkpoints</span></div><p class="text-xs text-slate-500 dark:text-slate-400">Restoring creates a new draft revision, so published work remains protected.</p><div class="space-y-2 max-h-[55vh] overflow-y-auto">${rows}</div></div>`);
