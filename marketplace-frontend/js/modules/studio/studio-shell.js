...
const STUDIO_PREMADE_ELEMENTS = [
  ['sale-badge','Sale badge','Offers','badge','SALE EVENT'], ['price-badge','Price badge','Offers','badge','$29,995'], ['payment-card','Payment card','Offers','card','$499/mo'], ['apr-badge','APR badge','Offers','badge','9.99% APR'], ['clearance-ribbon','Clearance ribbon','Offers','ribbon','CLEARANCE'], ['new-arrival','New arrival','Offers','badge','JUST ARRIVED'], ['hot-deal','Hot deal','Offers','badge','HOT DEAL'], ['price-drop','Price drop','Offers','badge','PRICE DROP'],
  ...
  ['review-badge','Review badge','Trust','trust','4.9 ★ VERIFIED REVIEWS'], ['guarantee-badge','Guarantee badge','Trust','trust','100% GUARANTEE'], ['one-owner','One owner badge','Trust','badge','ONE OWNER'], ['certified-badge','Certified badge','Trust','badge','CERTIFIED PRE-OWNED'], ['no-credit','No credit impact','Trust','trust','NO CREDIT IMPACT'], ['fast-approval','Fast approval','Trust','trust','2-MIN PRE-APPROVAL'],
  ['trade-callout','Trade-in callout','Automotive','callout','TOP TRADE VALUE'], ['inventory-label','Inventory label','Automotive','badge','IN STOCK'], ['vehicle-specs','Vehicle specs','Automotive','card','2024 • SUV • AWD'], ['featured-vehicle','Featured vehicle','Automotive','card','FEATURED VEHICLE'], ['electric-label','Electric label','Automotive','badge','100% ELECTRIC'], ['fuel-saver','Fuel saver','Automotive','badge','FUEL EFFICIENT'],
  ['dealer-header','Dealership header','Brand','header','MARKETSYNC MOTORS'], ['hours-card','Hours card','Brand','card','OPEN TODAY 9–7'], ['contact-card','Contact card','Brand','card','CALL OUR TEAM'], ['location-card','Location card','Brand','card','VISIT OUR SHOWROOM'], ['social-follow','Social follow','Brand','button','FOLLOW US'], ['newsletter','Newsletter CTA','Brand','button','GET OUR SPECIALS'],
...
const STUDIO_ICON_LIBRARIES = { lucide: ['Lucide', 'lucide'], phosphor: ['Phosphor', 'ph'], tabler: ['Tabler', 'tabler'], material: ['Material Symbols', 'material-symbols'], heroicons: ['Heroicons', 'heroicons-outline'], bootstrap: ['Bootstrap Icons', 'bi'], remix: ['Remix Icon', 'ri'], iconoir: ['Iconoir', 'iconoir'], boxicons: ['Boxicons', 'bxs'], iconify: ['Iconify · Lucide set', 'lucide'] };
...
function studioIconUrl(name, library = 'lucide') { const [label, prefix] = STUDIO_ICON_LIBRARIES[library] || STUDIO_ICON_LIBRARIES.lucide; const slug = STUDIO_ICON_SLUGS[library]?.[name] || name; return `https://api.iconify.design/${encodeURIComponent(prefix)}/${encodeURIComponent(slug)}.svg?color=%23FFFFFF`; }
function renderStudioIconLibrary(query = '', library = document.getElementById('studio-icon-library-select')?.value || 'lucide') { return studioCatalogButtons(STUDIO_ICON_LIBRARY, query, item => { const fallback = STUDIO_ICON_SYMBOLS[item.name] || '✦'; const url = studioIconUrl(item.name, library); return `<button type="button" onclick="studioAddIcon('${item.id}')" title="${escS(item.label)}" class="aspect-square rounded-xl bg-slate-900/95 hover:bg-blue-600/30 border border-white/10 hover:border-blue-400/70 flex flex-col items-center justify-center gap-1 text-white shadow-sm transition"><span class="w-10 h-10 rounded-lg bg-slate-950/90 border border-white/10 flex items-center justify-center"><img src="${url}" alt="" loading="lazy" class="w-7 h-7 object-contain" onerror="this.classList.add('hidden');this.nextElementSibling.classList.remove('hidden')"><span class="hidden text-2xl leading-none" aria-hidden="true">${fallback}</span></span><span class="text-[8px] text-slate-200 truncate w-full px-1">${escS(item.label)}</span></button>`; }); }
...
function studioAddIcon(id) { const item = STUDIO_ICON_LIBRARY.find(icon => icon.id === id); const adapter = window.__studioAdapter; if (!item || !adapter) return; const library = document.getElementById('studio-icon-library-select')?.value || 'lucide'; const url = studioIconUrl(item.name, library); adapter.addImage(url, `Icon: ${item.name}`).then((image) => { if (image) { image.msData = { ...(image.msData || {}), iconName: item.name, iconLibrary: library, mediaType: 'svg-icon' }; adapter.saveHistory(); if (typeof showToast === 'function') showToast(`${item.name} icon added`, 'success'); return; } adapter.addText(STUDIO_ICON_SYMBOLS[item.name] || '✦', { fontSize: 110, fontWeight: '900', name: `Icon: ${item.name}`, iconName: item.name, iconLibrary: library }); if (typeof showToast === 'function') showToast('Icon preview unavailable — added emoji fallback', 'info'); }); }
...
async function searchStudioGifs() {
  ...
  const giphyKey = window.MS_GIPHY_API_KEY || '';
  const tenorKey = window.MS_TENOR_API_KEY || '';
  if ((provider === 'giphy' && !giphyKey) || (provider === 'tenor' && !tenorKey)) {
    results.innerHTML = `<div class="col-span-3 rounded-xl border border-amber-400/30 bg-amber-400/10 p-3 text-xs text-amber-100"><strong>${provider === 'giphy' ? 'GIPHY' : 'Tenor'} search is ready but needs its free API key.</strong><br><span class="text-amber-200/70">Set window.MS_${provider === 'giphy' ? 'GIPHY' : 'TENOR'}_API_KEY in staging to load live results. Featured GIFs and pasted URLs work now.</span></div>`;
    return;
  }
  try {
    const endpoint = provider === 'giphy'
      ? `https://api.giphy.com/v1/gifs/search?api_key=${encodeURIComponent(giphyKey)}&q=${encodeURIComponent(query)}&limit=24&rating=pg-13`
      : `https://tenor.googleapis.com/v2/search?key=${encodeURIComponent(tenorKey)}&client_key=marketsync-studio&q=${encodeURIComponent(query)}&limit=24&contentfilter=medium`;
    const response = await fetch(endpoint);
    if (!response.ok) throw new Error('Provider search failed');
    const payload = await response.json();
    const items = provider === 'giphy' ? (payload.data || []).map(item => ({ title: item.title || query, url: item.images?.original?.url || item.images?.fixed_width?.url })) : (payload.results || []).map(item => ({ title: item.content_description || query, url: item.media_formats?.gif?.url || item.media_formats?.mediumgif?.url }));
    results.innerHTML = items.filter(item => item.url).map(item => `<button type="button" onclick="addStudioGifFromUrl('${String(item.url).replace(/'/g, '&#39;')}')" title="${escS(item.title)}" class="rounded-xl overflow-hidden border border-white/10 bg-slate-900 hover:border-blue-400/70 text-left"><img src="${item.url}" alt="" loading="lazy" class="w-full aspect-video object-cover"><span class="block p-1.5 text-[9px] text-slate-200 truncate">${escS(item.title)}</span></button>`).join('') || '<div class="col-span-3 p-3 text-center text-xs text-slate-400">No GIFs found. Try another search.</div>';
  } catch (error) {
    results.innerHTML = '<div class="col-span-3 p-3 text-center text-xs text-rose-300">GIF search could not load. Check the provider key or paste a GIF URL below.</div>';
  }
}
window.searchStudioGifs = searchStudioGifs; window.studioGifPresetSearch = studioGifPresetSearch;
function renderStudioPremadeElements(query = '', category = 'all') { const needle = String(query || '').toLowerCase(); return STUDIO_PREMADE_ELEMENTS.filter(item => (!needle || `${item.name} ${item.category} ${item.text}`.toLowerCase().includes(needle)) && (category === 'all' || item.category === category)).map(item => `<button type="button" onclick="studioAddPremade('${item.id}')" class="text-left p-3 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-indigo-500/20 border border-slate-300 dark:border-slate-700 transition"><div class="flex items-center justify-between gap-2"><span class="text-[9px] uppercase tracking-wider font-black text-sky-400">${item.category}</span><span class="text-[9px] text-slate-500">＋</span></div><div class="mt-2 rounded-lg px-2 py-3 text-center font-black text-xs ${item.kind === 'legal' ? 'bg-slate-900 text-slate-300 text-[9px]' : item.kind === 'button' ? 'bg-blue-600 text-white' : item.kind === 'ribbon' ? 'bg-rose-600 text-white -rotate-2' : 'bg-indigo-500/20 text-white border border-indigo-400/40'}">${escS(item.text)}</div><div class="mt-2 text-[10px] font-bold truncate">${escS(item.name)}</div></button>`).join('') || '<div class="col-span-2 p-4 text-center text-xs text-slate-500">No elements match.</div>'; }
...
const STUDIO_TEXT_TEMPLATES = [
  ...
  ['sale-price','Sale price','Offers','$29,995','#10B981','#FFFFFF',56,'900'], ['monthly-price','Monthly payment','Offers','$499 / MONTH','#2563EB','#FFFFFF',42,'900'], ['apr-callout','APR callout','Offers','9.99% APR','#F59E0B','#172033',42,'900'], ['limited-time','Limited time','Offers','LIMITED TIME ONLY','#E11D48','#FFFFFF',34,'900'],
  ...
  ['info-card','Info card','Information','OPEN TODAY 9–7','#1E293B','#FFFFFF',25,'800'], ['phone-card','Contact card','Information','CALL OUR TEAM','#0F172A','#FFFFFF',25,'800'], ['location-card','Location card','Information','VISIT OUR SHOWROOM','#334155','#FFFFFF',25,'800'], ['disclaimer','Disclaimer','Information','PLUS TAXES, LICENSING & FEES','#F1F5F9','#334155',17,'700']
...
function studioAddPremade(id) { const item = STUDIO_PREMADE_ELEMENTS.find(element => element.id === id); const adapter = window.__studioAdapter; const canvas = adapter?.fabricCanvas; if (!item || !adapter || !canvas) return; const center = canvas.getCenter(); const objects = []; const fill = item.kind === 'button' ? '#2563EB' : item.kind === 'ribbon' ? '#E11D48' : item.kind === 'legal' ? '#172033' : '#4F46E5'; adapter.addShape(item.kind === 'badge' || item.kind === 'ribbon' || item.kind === 'button' ? 'badge' : 'rect', fill); const shape = canvas.getActiveObject(); if (shape) { shape.set({ left: center.left - 180, top: center.top - 42, width: item.kind === 'legal' ? 520 : 360, height: item.kind === 'legal' ? 70 : 84, rx: item.kind === 'button' ? 18 : 16 }); shape.msData = { ...(shape.msData || {}), name: item.name, premadeId: item.id }; objects.push(shape); } adapter.addText(item.text, { x: center.left - 150, y: center.top - 18, width: 300, fontSize: item.kind === 'legal' ? 16 : 27, fontWeight: item.kind === 'legal' ? '500' : '900', fill: item.kind === 'legal' ? '#CBD5E1' : '#FFFFFF' }); const text = canvas.getActiveObject(); if (text) { text.msData = { ...(text.msData || {}), name: `${item.name} text`, premadeId: item.id }; objects.push(text); } if (objects.length > 1) { canvas.discardActiveObject(); canvas.setActiveObject(new window.fabric.ActiveSelection(objects, { canvas })); adapter.groupSelected(); } if (typeof showToast === 'function') showToast(`${item.name} added — edit or duplicate it`, 'success'); }
...
function templatePreviewMarkup(tmpl) {
  ...
  const nodes = elements.map(element => {
    ...
    if (element.type === 'shape') return `<div style="${base}background:${escS(element.fill || '#2563eb')};border-radius:${Math.min(50, Number(element.rx || 0) / Math.max(1, Number(element.width || width)) * 100)}%;"></div>`;
    if (element.type === 'text') return `<div style="${base}color:${escS(element.fill || '#fff')};font-size:${Math.max(6, Math.min(28, Number(element.fontSize || 24) * 0.22))}px;font-weight:${escS(element.fontWeight || '700')};line-height:1.05;overflow:hidden;">${escS(previewText(element.text))}</div>`;
    return '';
  }).join('');
  ...
  if (tool === 'templates') {
    ...
    return `
      <div class="p-4 space-y-3">
        ...
        <div class="border-t border-slate-200 dark:border-slate-800 pt-3 space-y-2"><div class="flex items-center justify-between"><h4 class="text-[11px] font-black uppercase tracking-wider text-sky-400">GIF library · 100+ searches</h4><span class="text-[9px] text-slate-500">GIPHY · Tenor</span></div><div class="grid grid-cols-2 gap-2">${STUDIO_GIF_LIBRARY.map(([name,url]) => `<button type="button" onclick="addStudioGifFromUrl('${url}')" class="rounded-xl overflow-hidden border border-white/10 bg-slate-900 hover:border-blue-400/70"><img src="${url}" alt="${name}" loading="lazy" class="w-full aspect-video object-cover"><span class="block p-1 text-[9px] font-bold truncate text-slate-200">${name}</span></button>`).join('')}</div><div class="grid grid-cols-[1fr_auto] gap-2"><input id="studio-gif-search" onkeydown="if(event.key==='Enter') searchStudioGifs()" placeholder="Search 120+ GIF topics…" class="w-full px-3 py-2 rounded-xl bg-slate-900 text-white placeholder:text-slate-500 border border-white/10 text-xs"><select id="studio-gif-provider" class="px-2 rounded-xl bg-slate-900 text-white border border-white/10 text-xs"><option value="giphy">GIPHY</option><option value="tenor">Tenor</option></select></div><button onclick="searchStudioGifs()" class="w-full py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-black">Search GIFs</button><div class="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto">${STUDIO_GIF_PRESETS.map(item => `<button type="button" onclick="studioGifPresetSearch('${item.query.replace(/'/g, "\\'")}')" class="px-2 py-1 rounded-lg bg-slate-900 border border-white/10 text-[9px] text-slate-300 hover:text-white hover:border-blue-400/60">${escS(item.label)}</button>`).join('')}</div><div id="studio-gif-results" class="grid grid-cols-3 gap-2 max-h-[38vh] overflow-y-auto"></div><input id="studio-gif-url" placeholder="Or paste any GIF URL" class="w-full px-3 py-2 rounded-xl bg-slate-900 text-white placeholder:text-slate-500 border border-white/10 text-xs"><button onclick="addStudioGifFromUrl()" class="w-full py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-xs font-black">Add pasted GIF</button></div>
      </div>
    `;
  } else if (tool === 'icons') {
