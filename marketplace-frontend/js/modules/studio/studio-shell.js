...
function editStudioStructure(index) {
  ...
  crmOverlay(`<form onsubmit="event.preventDefault(); saveStudioStructure(${index})" class="p-5 space-y-4 max-w-md"><div><h3 class="text-lg font-black text-slate-900 dark:text-white">${item.type === 'repeater' ? 'Repeater mapping' : 'Component settings'}</h3><p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Configure the reusable definition used by this design.</p></div><label class="block text-xs font-bold text-slate-600 dark:text-slate-300">Name<input id="studio-structure-name" value="${escS(item.name || '')}" class="mt-1 w-full px-3 py-2 rounded-xl bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white"></label>${item.type === 'repeater' ? `<label class="block text-xs font-bold text-slate-600 dark:text-slate-300">Collection<select id="studio-structure-collection" class="mt-1 w-full px-3 py-2 rounded-xl bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white"><option value="inventory" ${item.collection === 'inventory' ? 'selected' : ''}>Inventory</option><option value="customers" ${item.collection === 'customers' ? 'selected' : ''}>Customers</option><option value="services" ${item.collection === 'services' ? 'selected' : ''}>Service offers</option></select></label><div><div class="text-xs font-bold text-slate-600 dark:text-slate-300 mb-2">Mapped fields</div><div class="grid grid-cols-2 gap-2">${fields.map(field => `<label class="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400"><input type="checkbox" value="${field}" ${item.template?.fields?.includes(field) ? 'checked' : ''}>${field}</label>`).join('')}</div></div>` : ''}<div class="flex justify-end gap-2"><button type="submit" class="px-4 py-2 rounded-xl bg-indigo-600 text-xs font-black text-white">Save mapping</button></div></form>`);
}
...
async function openStudioRevisionHistory() {
  ...
  try {
    ...
    const rows = revisions.length ? revisions.map(rev => `<div class="flex items-center justify-between gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700"><div class="min-w-0"><div class="text-sm font-bold text-slate-900 dark:text-white">Revision ${rev.revision_number}</div><div class="text-[11px] text-slate-500 dark:text-slate-400 truncate">${escS(rev.change_summary || 'Saved draft')} · ${new Date(rev.created_at).toLocaleString()}</div></div><button type="button" onclick="restoreStudioRevision('${rev.id}')" class="shrink-0 px-3 py-1.5 rounded-lg bg-slate-200 dark:bg-slate-700 hover:bg-indigo-600 hover:text-white text-xs font-bold text-slate-900 dark:text-white">Restore</button></div>`).join('') : '<p class="text-sm text-slate-400">No saved revisions yet.</p>';
