...
function pulsePerformancePanel(d, deptKeys) {
  const cards = deptKeys.map(key => {
    const config = PULSE_GAMIFICATION_DEPTS[key];
    const dept = d?.gamification?.departments?.[key];
    if (!config) return '';
    if (!dept) {
      return `<div class="p-4 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/20">
    ...
    return pulseLeaderboardCard(d?.gamification, key, { title: `${config.label} leaderboard`, tier: 'feature', limit: 7 });
  }).filter(Boolean).join('');
