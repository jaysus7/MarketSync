...
async function wsSaveGroupGovernance(btn) {
  ...
  if (btn) { btn.disabled = true; btn.textContent = 'Saving…'; }
  try {
    const result = await apiSendJson('/dealership/site/group-governance', 'PATCH', { locked_fields: fields, approval_required: approvalRequired });
    __wsGovernance = { ...__wsGovernance, inherited_locked_fields: result.locked_fields || fields, inherited_approval_required: result.approval_required === true, locked_fields: [...new Set([...(result.locked_fields || fields), ...(__wsGovernance.local_locked_fields || [])])], approval_required: result.approval_required === true || __wsGovernance.approval_required === true };
    renderWsBody();
